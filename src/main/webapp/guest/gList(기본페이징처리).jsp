...
<%
	...
	/* 페이징 처리
		...
		3. 총 페이지 수를 구한다. : totPage => 총 레코드 수 / 페이지 분량으로 하되, 나머지가 0이면 몫을, 0이 아니면 몫+1로 처리
...
<% if(pag != 1) { %>
	<a href="gList.jsp?pag=1"> 첫페이지로 </a>
<%} %>

<% if(pag > 1) { %>
	<a href="gList.jsp?pag=<%=pag-1%>"> 이전페이지로 </a>
<%} %>
<%=pag%>Page / <%=totPage %>Pages

<% if(pag < totPage) { %>
	<a href="gList.jsp?pag=<%=pag+1%>"> 다음페이지로 </a>
<%} %>
