...
<p><br/></p>
	<div class="container">
		<table class="table table-borderless">
			<tr>
				<td>
					<h2>게 시 판 검색 내역 리 스 트</h2>
					(<font color="blue"> ${searchTitle}</font>(으)로<font color="red">${searchString}</font>(을)를 검색한 결과 ${searchCount}건이 검색 되었습니다.)
				</td>
			</tr>
		</table>
	...
	<ul class="pagination justify-content-center">
		...
		<c:if test="${totPage != 0}">
		  ...
		  <c:if test="${pag != totPage}">
		    <li class="page-item"><a href="boSearch.bo?pag=${totPage}&pageSize=${pageSize}&search=${search}&searchString=${searchString}" title="마지막페이지" class="page-link text-secondary">▷▷</a>
		  </c:if>
		</c:if>
	</ul>
</div>
