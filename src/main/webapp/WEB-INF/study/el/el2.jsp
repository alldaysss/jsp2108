...
%>
	...
	atom4[0] lt atom4[1] : ${atom4[0] lt atom4[1]}<br/>
	atom4[0] <= atom4[1] : ${atom4[0] <= atom4[1]}<br/><br/>
	
	atom4[0] le atom4[1] : ${atom4[0] le atom4[1]}<br/>
	atom4[0] >= atom4[1] : ${atom4[0] >= atom4[1]}<br/><br/>
	
	atom4[0] ge atom4[1] : ${atom4[0] ge atom4[1]}<br/>
	atom4[0] != atom4[1] : ${atom4[0] != atom4[1]}<br/><br/>

	atom4[0] ne atom4[1] : ${atom4[0] ne atom4[1]}<br/>
	
		
	hobby : ${param.hobby}<br/>
	<%-- hobby[0] : ${param.hobby[0]}<br/>
	hobby[1] : ${param.hobby[1]}<br/> --%>
	<!-- JSTL을 사용하여 배열값을 변수에 저장 후 출력해야 한다, -->
...
%>
	...
	연산자(== 과 null을 동시에 비교) : ${var1 == "" || var == null} <br/>
	연산자(== 과 null을 동시에 비교는 EL에서는 empty) : ${empty var1} <br/>
	연산자(!= 과 not null을 동시에 비교는 EL에서는 not empty) : ${not empty var1} <br/>
	연산자(!= 과 not null을 동시에 비교는 EL에서는 !empty) : ${!empty var1} <br/>
	연산자(empty를 이용하면 ""나 null을 따로 비교할 필요가 없다.) : ${empty var2} <br/>
	연산자(empty를 이용하면 ""나 null을 따로 비교할 필요가 없다.) : ${empty var3} <br/>
