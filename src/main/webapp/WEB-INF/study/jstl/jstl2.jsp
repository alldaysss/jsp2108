...
<p><br/></p>
	<div class="container">
		...
		<p><b>조건문 : 참일때 수행내용만 기술 할 수 있다.(else문이 없다.)</b></p>
		<b>문법 : < c :if test="$ {조건식}">참일때수행내용< /c:if></b>
		...
		4. <c:if test="${su1 >= su2+0}">su1과 su2보다 크거나 같다.</c:if><br/>
		...
		<br/>
		<h3>반복문(forEach문)</h3>
		<p>문법 : < c :forEach var="변수" item="$ {배열/객체}">수행할내용</c :forEach> </p>
		<p>문법 : < c :forEach var="변수" begin="초기값" end="최종값" step="증감값" varStatus="인자">수행할내용</c :forEach> </p>
		<p>1.반복문예<br/>
...
%>
	...
	<hr/>
	<p>6. 반복문예(member객체이용) - 첫번째와 마지막 자료의 이름과 나이를 출력하시오.<br/>
				<c:forEach var="vo" items="${vos}" varStatus="st">
					<c:if test="${st.first}">첫번째 자료 :  ${st.first} : ${vo.name} / ${vo.age}<br/></c:if>
					<c:if test="${st.last}">마지막 자료 : ${st.last} : ${vo.name} / ${vo.age}<br/></c:if>
				</c:forEach>
	</p>
	<hr/>
	...
	<p>8. 반복문예(member객체이용) - 짝수번째 자료만 출력(begin~end)<br/>
				...
				<c:forEach var="vo" begin="0" end="${vosCnt-1}" varStatus="st" items="${vos}">
					<c:if test="${st.count % 2 == 0}">${st.count} : ${vo.name} / ${vo.age}  <br/></c:if>
				</c:forEach>
	</p>
	<hr/>
...
%>
	<p>토큰(특정 기호값)을 이용한 분리 : forTokens
		취미 : 
	...
	<hr/>
	<br/>
	<p>문제 : 취미중 바둑은 빨간색으로 수영은 파란색으로 출력하시오 
		취미 : 
		<c:forTokens var="hobby" items="${hobbys}" delims="/">
