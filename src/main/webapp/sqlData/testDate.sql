...
select concat(year(now()),'-',month(now()),'-',day(now())) from board order by idx desc;
...
/* 오늘은 무슨요일?(인덱스 값으로 반환됨 : 일-1, 월-2, 화-3, 수-4, 목-5, 금-6, 토-7) */
...
/* 해당월의 마지막일(Last_Day)? : (2021. 02. 29) 평년의 2월 28, 윤년의 2월 29 */
...
/*
  %Y : 4자리 년도, %y 2자리 년도
  %M : 영문으로 긴월, %b 영문 짧은 월
  %m : 숫자 월 두자리로 표시, %c : 숫자월(한자리 월은 한자리로 표시)
...
/* 게시글의 글중 24시간 이내의 자료만 출력하시오 (date_add())*/
select nickName, wDate from board where date_add(now(), interval -24 hour) < wDate;

/* 게시글의 글중 24시간 이내의 자료만 출력하시오 (date_sub())*/
select nickName, wDate from board where date_sub(now(), interval 3 hour) < wDate;