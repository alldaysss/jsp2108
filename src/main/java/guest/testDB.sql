...
alter table insarok change column name name not null varchar(20) not null;
...
alter talbe insa rename insarok;
...
select name as '성명', gender '성별', jikwi as '직위', address '주소' from insa;
...
-- '영업과', '남자'사원중 '인천'에살거나 '부산'에 사는 직원을 출력하시오?(또는 : or / in)
...
select * from insarok where gender='여자' and age>=20 and<=29;
...
-- 5번부터(6번째자료부터) 5건만(limit) 출력하시오?(시작은 0번레코드이다.)
