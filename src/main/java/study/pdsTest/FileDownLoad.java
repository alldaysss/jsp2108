...
//		String realPath = application.getRealPath("/data/pdsTest");
		...
		if(mimeType == null) {
			response.setContentType("application/octet-stream"); // 2진 바이너리 파일로 전송하겠다는것...
		}
		
		String downLoadName = "";
		if(request.getHeader("user-agent").indexOf("MSIE") == -1) {
			// 사용자 브라우저가 익스플로러가 아니라면?
		...
		// 다운로드 할 파일명과 형식을 헤더파일에 담아서 클라이언트에 전송한다.
		response.setHeader("Content-Disposition", "attachment;filename=" + downLoadName);
		
		// Java에 의해서 실제로 업(다운)로드를 하기위한 객체(FileInputStram/FileOutputStream)를 생성한다.
		FileInputStream fileInputStream = new FileInputStream(File);
		ServletOutputStream servletOutputStream = response.getOutputStream();
		
		// 생성된 객체에 실제로 전송할 파일을 전송처리한다.
