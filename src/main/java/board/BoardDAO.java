...
public class BoardDAO {
	...
	public List<BoardVO> getBoardList(int startIndexNo, int pageSize) {
		...
		try {
			...
			while(rs.next()) {
				...
				// 날짜를 24시간제로 체크하기 위해서 사용자 정의 메소드로 만든 timeDiff()를 사용한다.
				vo.setwDate(rs.getString("wDate")); // 만약 w Date가 날짜타입일경우...
	...
	public int setBoInput(BoardVO vo) {
		int res = 0;
		try {
			sql = "insert into board value(default,?,?,?,?,?,default,default,?,default,?)";
			pstmt = conn.prepareStatement(sql);
	...
	public int totRecCnt(String search, String searchString) {
		...
		try {
			...
			System.out.println("SQL 에러(cnt) : " + e.getMessage());
		} finally {
			getconn.rsClose();
		}
		return totRecCnt;
